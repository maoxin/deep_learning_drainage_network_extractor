...
from models.deep_lab_v3plus.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
...
class DRN_A(nn.Module):
    ...
    def __init__(self, block, layers, BatchNorm=None, in_channels=3):
        self.inplanes = 64
...
def drn_a_50(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN_A(Bottleneck, [3, 4, 6, 3], BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
    #     model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model


def drn_c_26(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch='C', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_c_42(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(BasicBlock, [1, 1, 3, 4, 6, 3, 1, 1], arch='C', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_c_58(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='C', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_d_22(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch='D', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_d_24(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 2, 2], arch='D', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_d_38(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(BasicBlock, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_d_40(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(BasicBlock, [1, 1, 3, 4, 6, 3, 2, 2], arch='D', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_d_54(BatchNorm, in_channels=3):
             # pretrained=True):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
def drn_d_105(BatchNorm, in_channels=3):
              # pretrained=True):
    model = DRN(Bottleneck, [1, 1, 3, 4, 23, 3, 1, 1], arch='D', BatchNorm=BatchNorm, in_channels=in_channels)
    # if pretrained:
...
if __name__ == "__main__":
    import torch
    # model = drn_a_50(BatchNorm=nn.BatchNorm2d, in_channels=3)
    #                  pretrained=True)
    model = drn_d_54(BatchNorm=nn.BatchNorm2d, in_channels=3)
    # pretrained=True)
